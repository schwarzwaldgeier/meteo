...
function data_now() {
		$ws = 0;
		$wd = 0;
    	$query = "SELECT wind_speed,wind_direction,tstamp from weather_merkur2 order by uid desc limit 1";
...
function data_avg($mins) {
    	...
    	$query = "SELECT wind_speed, wind_direction from weather_merkur2 where tstamp > $mytime";
    	$void = mysql_select_db($db);
    	$result = mysql_query($query);
    	$anzkomplett = @mysql_num_rows($result);
    	$x = $anzkomplett - 1;
...
function data_1hourmax() {
		...
		$wd = 0;
    	$mytime = time() - 60 * 20;
    	$query = "SELECT max(wind_maxspeed) as wind_speed,wind_direction from weather_merkur2 where tstamp > $mytime group by wind_direction order by wind_speed desc limit 1";
    	$void = mysql_select_db($db);
    	$result = mysql_query($query);
    	$anzkomplett = @mysql_num_rows($result);
    	$x = $anzkomplett - 1;
